...
- **WHEN** both getkbd instances display the same short confirmation code and
  the user confirms it on both Macs
- **THEN** each instance SHALL persist the paired peer identity and show the
  peer as available for verification
...
- **WHEN** the user changes the monitor input and the same hub disappears from
  one Mac and appears on the other
